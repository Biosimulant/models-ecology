...
class Pfeiffer2001AtpProducingpathwaysCooperationcompetition(biosim.BioModule):
    ...
    def __init__(self, model_path: str = "data/BIOMD0000000337.xml", min_dt: float = 0.1) -> None:
        if min_dt <= 0:
    ...
    def outputs(self) -> Set[str]:
        return {"resource_state", "community_state", "cooperation_metrics"}
    ...
    def _publish_outputs(self, t: float) -> None:
        ...
        self._outputs = {
            "resource_state": BioSignal(
                source=source_name,
                name="resource_state",
                value={"substrate_resource": latest["S"]},
                time=float(t),
                metadata=SignalMetadata(
                    units=self._species_units.get("S"),
                    description="Shared substrate resource concentration from the Pfeiffer2001 SBML model.",
                    kind="state",
                ),
            ),
            "community_state": BioSignal(
                ...
                time=float(t),
                metadata=SignalMetadata(
                    units=self._species_units.get("N1") or self._species_units.get("N2"),
                    description="Population sizes for the two ATP-pathway strategies encoded by the model.",
                    kind="state",
                ),
            ),
            "cooperation_metrics": BioSignal(
                source=source_name,
                name="cooperation_metrics",
                value={
                ...
                metadata=SignalMetadata(
                    units=None,
                    description="Strategy fractions and resource intensity for the ATP-pathway competition model.",
                    kind="summary",
                ),
            ),
        }
