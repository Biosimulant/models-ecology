...
def test_instantiation(biosim):
    ...
    assert set(outputs) == {"prey_population_state", "predator_population_state"}
...
def test_visualize_returns_expected_lv_visuals(biosim):
    ...
    assert visuals[2]["data"]["columns"] == ["Metric", "Value", "Unit"]
...
def test_lab_level_io_maps_to_internal_model_outputs():
    lab_manifest = Path(__file__).resolve().parents[2] / "lab.yaml"
