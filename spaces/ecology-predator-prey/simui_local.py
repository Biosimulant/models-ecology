#!/usr/bin/env python3
"""Run ecology-predator-prey with SimUI."""
...
def _repo_root_map(current_repo_root: Path) -> dict[str, Path]:
    return {"Biosimulant/models-ecology": current_repo_root.resolve()}
...
def main() -> None:
    ...
    import biosim
    from biosim.simui import Button, EventLog, Interface, Number, VisualsPanel
    ...
    ui = Interface(
        world,
        title=space.get("title", "Ecology: Predator-Prey"),
        description=space.get("description", "Classic predator-prey dynamics."),
        controls=[
